SELECT   
  ...
  SUM (f.a_logicalvolume_size) disk_size,
  COUNT (DISTINCT (f.cmdb_id)) disk_count,
  COUNT (DISTINCT (i.cmdb_id)) AS interface_count

FROM 
  cdm_vmware_esx_server_1 s
  INNER JOIN cdm_virtualization_layer_1 a ON LOWER (s.cmdb_id) = a.a_root_container   
  LEFT OUTER JOIN cdm_cpu_1 c ON LOWER (s.cmdb_id) = c.a_root_container   
  LEFT OUTER JOIN cdm_usage_1 u ON LOWER (s.cmdb_id) = LOWER(u.end1_id) left outer join cdm_vmware_datastore_1 f ON LOWER (u.end2_id) = lower(f.cmdb_id)   
